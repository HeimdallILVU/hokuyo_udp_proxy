...
struct ScannerConfig {
    std::string receive_on_ip;
    int send_to_port;
    std::string sim_receive_on_ip;
    int sim_hokuyo_target_port;
};
...
void run(const ScannerConfig& config) {
    std::cout << "Starting scanner: " << config.receive_on_ip << " " << config.send_to_port << std::endl;
    ...
    inet_pton(AF_INET, config.receive_on_ip.c_str(), &hokuyo_addr.sin_addr);
    
    sim_hokuyo_addr.sin_family = AF_INET;
    sim_hokuyo_addr.sin_port = htons(config.sim_hokuyo_target_port);
    inet_pton(AF_INET, config.sim_receive_on_ip.c_str(), &sim_hokuyo_addr.sin_addr);
        ...
        if (ret > 0) {
            ...
            if (fds[0].revents & POLLIN) {
                ...
                if (!sim_mode.load()) {
                    if (recv_len > 0) {
                        ...
                        aggregated_data = process_data(buffer, aggregated_data, relay_sock, config.send_to_port);
                    }
                }
            }
            ...
            if (fds[1].revents & POLLIN) {
                if (!sim_mode.load() && std::chrono::duration_cast<std::chrono::seconds>(current_time - last_sim_switch_time).count() > 0.5) {
                    std::cout << "Switching to sim mode for " << config.receive_on_ip << std::endl;
                    sim_mode.store(true);
                }
                ...
                if (recv_len > 0) {
                    buffer.resize(recv_len);
                    sim_aggregated_data = process_data(buffer, sim_aggregated_data, relay_sock, config.send_to_port);
                }
            }
        }
        
        if (sim_mode.load() && std::chrono::duration_cast<std::chrono::seconds>(std::chrono::steady_clock::now() - last_sim_switch_time).count() > 10) {
            std::cout << "Switching back to real mode for " << config.receive_on_ip << std::endl;
...
std::vector<ScannerConfig> read_config_file(const std::string& filename) {
    ...
    if (config["scanners"]) {
        for (const auto& scanner : config["scanners"]) {
            ScannerConfig sc;
            sc.receive_on_ip = scanner["receive_on_ip"].as<std::string>();
            sc.send_to_port = scanner["send_to_port"].as<int>();
            sc.sim_receive_on_ip = scanner["sim_receive_on_ip"].as<std::string>();
            sc.sim_hokuyo_target_port = scanner["sim_hokuyo_target_port"].as<int>();
...
int main(int argc, char* argv[]) {
    ...
    std::vector<ScannerConfig> configs;
    try {
        configs = read_config_file(argv[1]);
    ...
    for (const auto& config : configs) {
        threads.emplace_back(run, config);
    }
